    ...
    <Layout>
      <Flex paddingTop={2} paddingBottom={2} paddingLeft={{ base: 2, md: 0 }} paddingRight={{ base: 2, md: 0 }} justifyContent="center">
        <Box width="1180px">
          ...
          <Box>
            <Box marginBottom={1}>
              {/* ユーザープロファイルコンテナ　ユーザー情報を表示。useUserで常に最新のデータを表示する */}
              <UserProfileContainer userId={id} user={user} />
            </Box>
            <Box marginBottom={1}>
              <Separator />
            </Box>
            {/* ユーザー商品カードリストコンテナ　所持する商品カードリストを表示。userSearchで常に最新のデータを取得する */}
...
export const getStaticProps = async ({ params }: GetStaticPropsContext) => {
  ...
  // ユーザー情報と所持する商品を取得、静的ページを作成、10秒でrevalidateな状態にして静的ページを更新
