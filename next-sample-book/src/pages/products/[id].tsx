import type { GetStaticPaths, GetStaticProps, GetStaticPathsContext, InferGetStaticPropsType, NextPage } from "next";
      ...
      >
        <Box>
          <Breadcrumb>
            ...
            <BreadcrumbItem>
              <Link href="/search"><a>検索</a></Link>
            </BreadcrumbItem>
            <BreadcrumbItem>
              <Link href={`/search/${product.category}`}><a>{categoryNameDict[product.category as Category]}</a></Link>
            </BreadcrumbItem>
          ...
          <Separator />
          <Box paddingTop={1}>
            <Text as="h2" variant="large" marginTop={0}>出品者</Text>
          ...
          <Flex justifyContent="space-between" flexDirection="column" height={{ base: '', md: '100%' }}>
            ...
            <Box>
              {product.description .split('\n') .map((text: string, i: number) => (
                <Text key={i} as="p">{text}</Text>
              ))}
            </Box>
            {/* カート追加ボタンコンテナ。ボタンが押されたらShoppingCartContextに商品を追加 */}
...
export const getStaticPaths: GetStaticPaths = async () => {
  const context: ApiContext = {
    apiRootUrl: process.env.API_BASE_URL || 'http://localhosts:5000',
  }
  // 商品からパスを生成
...
export const getStaticProps: GetStaticProps = async ({
  ...
  // 商品を取得し静的ページを作成、10秒でstaleな状態にし静的ページを更新する
